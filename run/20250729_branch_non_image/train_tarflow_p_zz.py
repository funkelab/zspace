...
if __name__ == "__main__":
    ...
    sample_dir = u.get_save_dir(c.tarflow_p_zz_model_name) / "xc_samples"
    ckpt_dir = u.get_save_dir(c.tarflow_p_zz_model_name) / "checkpoints"
    ...
    for epoch in tqdm(range(c.num_epochs_tarflow)):
        ...
        for batch_idx, batch in enumerate(train_loader):
                xb = batch[1]                        # shape : (batch_size, input_dims["xb"])
                xc = batch[2]                        # shape : (batch_size, input_dims["xc"])
                hidden_var = batch[3]                # shape : (batch_size)

                xb = xb.to(c.device)
                xc = torch.flatten(xc, start_dim=1, end_dim=-1).to(c.device)
                hidden_var = hidden_var.to(c.device)

                with torch.no_grad():
                    y, logvar_y = vae_model.encoders["xb"](xb)       # shape : (batch_size, z_dim)
                    x, logvar_x = vae_model.encoders["xc"](xc)       # shape : (batch_size, z_dim)

                x = x.view(xc.size(0), input_dim, c.token_size).to(c.device)   # shape : (batch_size, num_tokens, token_size)

                eps = c.noise_std * torch.randn_like(x)
                x = x + eps
                x = x.to(c.device)
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(valid_loader):
                    ...
                    with torch.no_grad():
                        y, logvar_y = vae_model.encoders["xb"](xb)   # shape : (batch_size, z_dim)
                        x, logvar_x = vae_model.encoders["xc"](xc)
        ...
        if (epoch + 1) % c.sample_freq == 0 or epoch == 0 or epoch == 99:
            u.sample_p_zz(valid_dataset, vae_model, tarflow_model, sample_dir, epoch=epoch)
            torch.save(tarflow_model.state_dict(), ckpt_dir / f"{c.tarflow_p_zz_model_name}_epoch_{epoch}.pth")
            tqdm.write(f"saved and sampled at epoch {epoch + 1}")

    print("training complete")
    u.plot_losses(train_losses, valid_losses, u.get_save_dir(c.tarflow_p_zz_model_name))