...
if __name__ == "__main__":
    ...
    sample_dir = u.get_save_dir(c.tarflow_p_xy_model_name) / "xc_samples"
    ckpt_dir = u.get_save_dir(c.tarflow_p_xy_model_name) / "checkpoints"
    ...
    for epoch in tqdm(range(c.num_epochs_tarflow)):
        ...
        if (epoch + 1) % c.sample_freq == 0 or epoch == 0 or epoch == 99:
            ...
            torch.save(tarflow_model.state_dict(), ckpt_dir / f"{c.tarflow_p_xy_model_name}_epoch_{epoch}.pth")
            tqdm.write(f"saved and sampled at epoch {epoch + 1}")

    print("training complete")
    u.plot_losses(train_losses, valid_losses, u.get_save_dir(c.tarflow_p_xy_model_name))